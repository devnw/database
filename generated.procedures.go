...
func (conn *dbconn) CreateScanSummary(_SourceID string, _OrgID string, _ScanID string, _ScanStatus string, _ScanClosePayload string, _ParentJobID string) (id int, affectedRows int, err error) {

	conn.Exec(&connection.Procedure{
		Proc:       "CreateScanSummary",
		Parameters: []interface{}{_SourceID, _OrgID, _ScanID, _ScanStatus, _ScanClosePayload, _ParentJobID},
		Callback: func(results interface{}, dberr error) {
			err = dberr
...
func (conn *dbconn) CreateTicket(_Title string, _Status string, _DetectionID string, _OrganizationID string, _DueDate time.Time, _UpdatedDate time.Time, _ResolutionDate time.Time) (id int, affectedRows int, err error) {

	conn.Exec(&connection.Procedure{
		Proc:       "CreateTicket",
		Parameters: []interface{}{_Title, _Status, _DetectionID, _OrganizationID, _DueDate, _UpdatedDate, _ResolutionDate},
		Callback: func(results interface{}, dberr error) {
			err = dberr
						...
						if err = rows.Err(); err == nil {
							...
							if err = rows.Scan(
								...
								newIgnore := &dal.Ignore{
									...
									Activevar:          myActive[0] > 0, // converts uint8 to bool
									Portvar:            myPort,
						...
						if err = rows.Err(); err == nil {
							...
							if err = rows.Scan(
								...
								newJobConfig := &dal.JobConfig{
									...
									Continuousvar:            myContinuous[0] > 0, // converts uint8 to bool
									WaitInSecondsvar:         myWaitInSeconds,
									MaxInstancesvar:          myMaxInstances,
									AutoStartvar:             myAutoStart[0] > 0, // converts uint8 to bool
...
func (conn *dbconn) GetAssetGroup(inOrgID string, _GroupID int, _ScannerSourceID string) (domain.AssetGroup, error) {
	var err error
						...
						if err = rows.Err(); err == nil {
							...
							if err = rows.Scan(
								...
								newPermission := &dal.Permission{
									UserIDvar:   myUserID,
									OrgIDvar:    myOrgID,
									Adminvar:    myAdmin[0] > 0,    // converts uint8 to bool
									Managervar:  myManager[0] > 0,  // converts uint8 to bool
									Readervar:   myReader[0] > 0,   // converts uint8 to bool
									Reportervar: myReporter[0] > 0, // converts uint8 to bool
						...
						if err = rows.Err(); err == nil {
							...
							if err = rows.Scan(
								...
								newSession := &dal.Session{
									...
									IsDisabledvar: myIsDisabled[0] > 0, // converts uint8 to bool
						...
						if err = rows.Err(); err == nil {
							...
							if err = rows.Scan(
								...
								newVulnerabilityInfo := &dal.VulnerabilityInfo{
									...
									Createdvar:         myCreated,
									Updatedvar:         myUpdated,
									NeedsUpdatevar:     myNeedsUpdate[0] > 0, // converts uint8 to bool
...
func (conn *dbconn) UpdateDetectionTimesSeen(_DeviceID string, _VulnID string, _TimesSeen int, _StatusID int) (id int, affectedRows int, err error) {

	conn.Exec(&connection.Procedure{
		Proc:       "UpdateDetectionTimesSeen",
		Parameters: []interface{}{_DeviceID, _VulnID, _TimesSeen, _StatusID},
		Callback: func(results interface{}, dberr error) {
			err = dberr
...
func (conn *dbconn) UpdateJobHistory(_ID string, _Status int, _ConfigID string, _Payload string, _UpdatedBy string) (id int, affectedRows int, err error) {

	conn.Exec(&connection.Procedure{
		Proc:       "UpdateJobHistory",
		Parameters: []interface{}{_ID, _Status, _ConfigID, _Payload, _UpdatedBy},
		Callback: func(results interface{}, dberr error) {
			err = dberr
...
func (conn *dbconn) UpdateTicket(_Title string, _Status string, _OrganizationID string, _UpdatedDate time.Time, _ResolutionDate time.Time) (id int, affectedRows int, err error) {

	conn.Exec(&connection.Procedure{
		Proc:       "UpdateTicket",
		Parameters: []interface{}{_Title, _Status, _OrganizationID, _UpdatedDate, _ResolutionDate},
		Callback: func(results interface{}, dberr error) {
			err = dberr
